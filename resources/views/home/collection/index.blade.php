                    ...
                    @foreach ($incomplete as $categoryId => $categorycollections)
                        <li class="nav-item">
                            <a class="nav-link {{ $loop->first ? 'active' : '' }}" id="categoryTab-{{ isset($categories[$categoryId]) ? $categoryId : 'misc' }}" data-toggle="tab"
                                href="#collectiontestincomp-{{ isset($categories[$categoryId]) ? $categoryId : 'misc' }}" role="tab">
            ...
            <div class="card-header">
                <ul class="nav nav-tabs card-header-tabs">
                    @foreach ($collections as $categoryId => $categorycollections)
                        <li class="nav-item">
                            <a class="nav-link {{ $loop->first ? 'active' : '' }}" id="categoryTab-{{ isset($categories[$categoryId]) ? $categoryId : 'misc' }}" data-toggle="tab"
                                href="#collectiontest-{{ isset($categories[$categoryId]) ? $categoryId : 'misc' }}" role="tab">
                                {!! isset($categories[$categoryId]) ? $categories[$categoryId]->name : 'Miscellaneous' !!}
                            </a>
                        </li>
                    @endforeach
