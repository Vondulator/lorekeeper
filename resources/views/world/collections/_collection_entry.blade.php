    ...
    <div class="col-lg-6 col-lg-10 mx-auto">
        ...
        <div class="card-body">
            @if ($collection->imageUrl)
                <div class="world-entry-image text-center mb-2"><a href="{{ $collection->imageUrl }}" data-lightbox="entry" data-title="{{ $collection->name }}"><img src="{{ $collection->imageUrl }}" class="world-entry-image mw-100"
                            style="max-height:300px;" /></a></div>
            @endif
            <div>
                <h1>
                    {!! $collection->displayName !!} @if (Auth::check() && Auth::user()->hasCollection($collection->id))
                        <i class="fas fa-check" data-toggle="tooltip" title="You've completed this collection."></i>
                    @endif
                </h1>
                ...
                <div class="alert alert-secondary">
                    ...
                    @if ($collection->parent_id && Auth::check() && !Auth::user()->hasCollection($collection->id))
                        ...
                        @php
                            if (Auth::check()) {
                                $completed = DB::table('user_collections')
                                    ->where('user_id', Auth::user()->id)
                                    ->where('collection_id', $collection->parent_id)
                                    ->count();
                            }
                        @endphp
