    ...
    <tbody id="rewardTableBody">
        @if($loots)
            @foreach($loots as $loot)
                <tr class="reward-row">
                    <td>{!! Form::select('award_type[]', ['Item' => 'Item', 'Currency' => 'Currency', 'Award' => ucfirst(__('awards.award'))] + ($showLootTables ? ['LootTable' => 'Loot Table'] : []) + ($showRaffles ? ['Raffle' => 'Raffle Ticket'] : []), $loot->type, ['class' => 'form-control award-type', 'placeholder' => 'Select Reward Type']) !!}</td>
