...
class HolService extends Service
{
    ...
    public function makeGuess($data, $user)
    {
        ...
        try {
            if(!in_array($data['guess'] ?? null, ['higher', 'lower'], true)) throw new \Exception('You must make a valid guess.');
            if(!isset($data['number']) || $data['number'] < 2 || $data['number'] > 12) throw new \Exception('This game has expired. Please start a new one.');
