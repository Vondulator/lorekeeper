    ...
    */
    ...
    public function createTradeListing($data, $user) {
        ...
        try {
            ...
            if (empty($listing->data['seeking']) && empty($listing->data['seeking_etc'])) {
                throw new \Exception("Please enter what you're seeking.");
            }
            if (empty($listing->data['offering']) && empty($listing->data['offering_etc'])) {
    ...
    public function markExpired($data, $user) {
        ...
        try {
            ...
            if ($listing->user_id != $user->id && !$user->hasPower('manage_submissions')) {
            ...
            if (isset($data['item_ids'])) {
                ...
                array_walk($data['item_ids'], function ($id, $key) use (&$keyed_quantities, $data) {
                    if ($id != null && !in_array($id, array_keys($keyed_quantities), true)) {
                        ...
                        if ($quantity < 1) {
                            throw new \Exception('Item quantities must be at least 1.');
                        }
        ...
        try {
            ...
            if (isset($data['stack_id'])) {
                foreach ($data['stack_id'] as $key=>$stackId) {
                    ...
                    if ($quantity < 1 || $quantity > $stack->count) {
                        throw new \Exception('One or more offered item quantities is invalid.');
                    }
