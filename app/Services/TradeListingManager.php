    ...
    */
    ...
    public function createTradeListing($data, $user)
    {
        ...
        try {
            ...
            if(($data['offering_etc'] ?? null) || ($data['seeking_etc'] ?? null)) {
                $listingData = $listing->data;
            ...
            if(empty($listing->data['seeking']) && empty($listing->data['seeking_etc'])) throw new \Exception("Please enter what you're seeking.");
            if(empty($listing->data['offering']) && empty($listing->data['offering_etc'])) throw new \Exception("Please enter what you're offering.");
    ...
    public function markExpired($data, $user)
    {
        ...
        try {
            ...
            if($listing->user_id != $user->id && !$user->hasPower('manage_submissions')) throw new \Exception("You can't edit this listing.");
            ...
            if(isset($data['item_ids'])) {
                ...
                array_walk($data['item_ids'], function($id, $key) use(&$keyed_quantities, $data) {
                    if($id != null && !in_array($id, array_keys($keyed_quantities), TRUE)) {
                        $quantity = (int) ($data['quantities'][$key] ?? 0);
                        if ($quantity < 1) throw new \Exception('Item quantities must be at least 1.');
                        $keyed_quantities[$id] = $quantity;
                    }
                });
        ...
        try {
            ...
            if(isset($data['stack_id'])) {
                foreach($data['stack_id'] as $key=>$stackId) {
                    ...
                    $quantity = (int) ($data['stack_quantity'][$stackId] ?? 0);
                    if ($quantity < 1 || $quantity > $stack->count) throw new \Exception('One or more offered item quantities is invalid.');
