    ...
    public function transferCharacterStack($sender, $recipient, $stacks, $quantities) {
        ...
        try {
            foreach ($stacks as $key=>$stack) {
                ...
                if (($recipient->logType == 'Character' && !$sender->hasPower('edit_inventories') && Auth::id() != $recipient->user_id) || ($recipient->logType == 'User' && !Auth::user()->hasPower('edit_inventories') && Auth::id() != $sender->user_id)) {
    ...
    public function transferStack($sender, $recipient, $stacks, $quantities) {
        ...
        try {
            foreach ($stacks as $key=>$stack) {
                ...
                if ($stack->award->user_limit && (($count + $quantity) > $stack->award->user_limit)) {
