            ...
            foreach([$users, $characters] as $targets){
                ...
                foreach($targets as $target){
                    foreach($awards as $award) {
                        if($this->creditAward($staff, $target, 'Staff Grant', Arr::only($data, ['data', 'disallow_transfer', 'notes']), $award, $keyed_quantities[$award->id])) {
                            if($type == "User"){
                        ...
                        {
                            throw new \Exception("Failed to credit ".__('awards.awards')." to ".$target->name.".");
                        }
                    }
                }
            }
    ...
    public function transferCharacterStack($sender, $recipient, $stacks, $quantities)
    {
        ...
        try {
            foreach($stacks as $key=>$stack) {
                ...
                if(($recipient->logType == 'Character' && !$sender->hasPower('edit_inventories') && Auth::id() != $recipient->user_id) || ($recipient->logType == 'User' && !Auth::user()->hasPower('edit_inventories') && Auth::id() != $sender->user_id)) throw new \Exception("Cannot transfer ".__('awards.awards')." to/from a ".__('lorekeeper.character')." you don't own.");
                ...
                $this->creditAward($sender, $recipient, $sender->logType == 'User' ? 'User â†’ '.ucfirst(__('lorekeeper.character')).' Transfer' : ucfirst(__('lorekeeper.character')).' â†’ User Transfer', $stack->data, $stack->award, $quantity);
    ...
    public function transferStack($sender, $recipient, $stacks, $quantities)
    {
        ...
        try {
            foreach($stacks as $key=>$stack) {
                ...
                if($stack->award->user_limit && (($count + $quantity) > $stack->award->user_limit)) throw new \Exception("Transfer failed: This would take the recipient over their limit for this ".__('awards.award').".");
