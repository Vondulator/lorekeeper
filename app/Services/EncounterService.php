...
class EncounterService extends Service
{
    ...
    public function takeAction($id, $data, $user)
    {
        ...
        try {
            ...
            if ((int) $area->id !== (int) $id->id || !$area->encounters()->where('encounter_id', $data['encounter_id'])->exists()) {
                throw new \Exception('Invalid encounter area.');
            }
            $action = EncounterPrompt::where('encounter_id', $data['encounter_id'])->find($data['action']);
            if (!$action) {
                abort(404);
            }
    ...
    public function grantRemoveEnergy($action, $user, $use_characters, $area, $character = null)
    {
        ...
        if ($use_energy) {
            ...
            $energyValue = (int) $action->extras['energy_value'];
            if ($action->extras['math_type'] === 'add') {
                $quantity = $recipient->encounter_energy + $energyValue;
            } elseif ($action->extras['math_type'] === 'subtract') {
                $quantity = $recipient->encounter_energy - $energyValue;
            } else {
                throw new \Exception('Invalid encounter energy action.');
            }
