    ...
    */
    ...
    public function createHunt($data, $user)
    {
        ...
        try {
            ...
            $hunt = ScavengerHunt::create(Arr::only($data, ['name', 'display_name', 'summary', 'clue', 'locations', 'start_at', 'end_at']));
    ...
    public function updateHunt($hunt, $data, $user)
    {
        ...
        try {
            ...
            $hunt->update(Arr::only($data, ['name', 'display_name', 'summary', 'clue', 'locations', 'start_at', 'end_at']));
    ...
    **********************************************************************************************/
    ...
    public function createTarget($data, $user)
    {
        ...
        try {
            ...
            if(!ScavengerHunt::whereKey($data['hunt_id'])->exists()) throw new \Exception('Invalid scavenger hunt.');
            if(HuntTarget::where('hunt_id', $data['hunt_id'])->count() >= 10) throw new \Exception ('This hunt already has the maximum number of targets.');
