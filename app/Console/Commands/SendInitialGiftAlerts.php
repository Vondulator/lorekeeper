...
class SendInitialGiftAlerts extends Command
{
    ...
    public function handle()
    {
        ...
        foreach($submissionCharacters as $submissionCharacter) {
            $characterDetails = Character::find($submissionCharacter);
            if (!$characterDetails || !$characterDetails->user_id) {
                continue;
            }
            $characterOwnerId = $characterDetails->user_id;
            ...
            foreach($notificationData as $data) {
                ...
                if (isset($decoded->character_id)) {
                    $notificationCharacter[] = $decoded->character_id;
                }
            }
            ...
            if($excludesSubmissionCharacter && $giftSubmissionCount != 0) {
                $characterOwnerData = User::find($characterOwnerId);
                if (!$characterOwnerData) {
                    continue;
                }

                Notifications::create('GIFT_SUBMISSION_ALERT', $characterOwnerData, [
                    'character_id' => $submissionCharacter,
                    'character_url' => $characterDetails->url,
