...
function getAssetModelString($type, $namespaced = true) {
    switch ($type) {
        ...
        case 'awards':
            return $namespaced ? \App\Models\Award\Award::class : 'Award';
        ...
        case 'user_awards':
            return $namespaced ? \App\Models\User\UserAward::class : 'UserAward';
...
function fillCharacterAssets($assets, $sender, $recipient, $logType, $data, $submitter = null) {
    ...
    foreach ($assets as $key => $contents) {
        if ($key == 'currencies' && count($contents)) {
            ...
            $service = new \App\Services\AwardCaseManager;
            foreach ($contents as $asset) {
                $awardRecipient = $asset['asset']->is_character_owned ? $recipient : $item_recipient;
                if (!$service->creditAward($sender, $awardRecipient, $logType, $data, $asset['asset'], $asset['quantity'])) {
                    return false;
                }
            }
        }
